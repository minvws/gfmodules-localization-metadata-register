...
@router.get("/meta/{service_name}",
            summary="Search for metadata for a specific service",
            tags=["metadata"]
            )
def get_metadata(
        service_name: str,
    ...
    entry = service.search(None, data_service, pseudo)
    if entry is None:
...
def get_deprecated_metadata(
        provider_id: str,
        service_name: str,
        pseudonym: str,
        service: MetadataService = Depends(container.get_deprecated_metadata_service)
) -> Any:
    span = trace.get_current_span()
