...
class ResourceEntry(Base):
    ...
    resource_type: Mapped[str] = mapped_column(
        "resource_type", String(64), nullable=False, index=True
    )
    resource_id: Mapped[str] = mapped_column(
        "resource_id", String(256), nullable=False, index=True
    )
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "created_dt", DateTime, nullable=False, default=datetime.utcnow
    )
